...
use crate::api::{EntriesReq, Entries, ProgressReq, ProgressMsg};
use crate::fs::route::CinemaFsRoute;
use crate::{Config, CinemaDb};
...
use fire_api::stream::{Streamer, StreamServer, StreamError};
...
pub async fn progress(
	...
	loop {
		...
		match msg {
			ProgressMsg::Movie(m) => {
				// get the data
				let mut entry = cinema.by_id_and_user(&m.id, &user.id).await?
					.ok_or_else(|| Error::Request("movie not found".into()))?;
			...
			ProgressMsg::Series(s) => {
				// get the data
				let mut entry = cinema.by_id_and_user(&s.id, &user.id).await?
					.ok_or_else(|| Error::Request("series not found".into()))?;
				...
				let episode = series.seasons.get_mut(s.season as usize)
					.and_then(|season| {
						season.episodes.get_mut(s.episode as usize)
					})
					.ok_or_else(|| Error::Request("episode not found".into()))?;
